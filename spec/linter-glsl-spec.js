...
const vsTest = async fileName => {
    const messages = await runLint(
        join(__dirname, "fixtures", "vert", fileName)
    );
...
const fsTest = async fileName => {
    const messages = await runLint(
        join(__dirname, "fixtures", "frag", fileName)
    );
    expect(messages.length).toEqual(3);
    expect(messages[0].severity).toEqual("error");
    expect(messages[0].excerpt).toEqual(
        "'vec5' : no matching overloaded function found"
    );
    expect(messages[1].severity).toEqual("error");
    expect(messages[1].excerpt).toEqual(
        "'assign' :  cannot convert from ' const float' to ' fragColor 4-component vector of float FragColor'"
    );
...
const gsTest = async fileName => {
    const messages = await runLint(
        join(__dirname, "fixtures", "geom", fileName)
    );
    expect(messages.length).toEqual(2);
    expect(messages[0].severity).toEqual("error");
    expect(messages[0].excerpt).toEqual(
        "'line_stripp' : unrecognized layout identifier, or qualifier requires assignment (e.g., binding = 4)"
    );
    expect(messages[1].severity).toEqual("error");
    expect(messages[1].excerpt).toEqual("'' : compilation terminated");
};
...
const tcTest = async fileName => {
    const messages = await runLint(
        join(__dirname, "fixtures", "tesc", fileName)
    );
    expect(messages.length).toEqual(2);
    expect(messages[0].severity).toEqual("error");
    expect(messages[0].excerpt).toEqual(
        "'verticaes' : there is no such layout identifier for this stage taking an assigned value"
    );
    expect(messages[1].severity).toEqual("error");
    expect(messages[1].excerpt).toEqual("'' : compilation terminated");
};
...
const teTest = async fileName => {
    const messages = await runLint(
        join(__dirname, "fixtures", "tese", fileName)
    );
...
const csTest = async fileName => {
    const messages = await runLint(
        join(__dirname, "fixtures", "comp", fileName)
    );
    expect(messages.length).toEqual(2);
    expect(messages[0].severity).toEqual("error");
    expect(messages[0].excerpt).toEqual(
        "'image variables not declared 'writeonly' and without a format layout qualifier' : not supported for this version or the enabled extensions"
    );
    expect(messages[1].severity).toEqual("error");
    expect(messages[1].excerpt).toEqual(
        "'compute shaders' : required extension not requested: GL_ARB_compute_shader"
    );
};

describe("linter-glslify", () => {
    beforeEach(async () => {
