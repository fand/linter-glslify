...
const shaderTypeLookup = (fieldName, fieldValue) => (shaderTypes.filter(shaderType => shaderType[fieldName] === fieldValue)[0]);
...
const parseGlslValidatorResponse = (inputs, output, firstRowRange) => new Promise((resolve) => {
    const toReturn = [];
...
exports.default = {
    ...
    activate() {
        ...
        state.subscriptions = new atom_1.CompositeDisposable();
        state.subscriptions.add(atom.config.observe('linter-glsl.linkSimilarShaders', (linkSimilarShaders) => {
            ...
            if (fs.existsSync(glslangValidatorPath) && fs.statSync(glslangValidatorPath).isFile()) {
                ...
                try {
                    state.glslangValidatorPath = which_1.default.sync(glslangValidatorPath);
                }
                catch (error) {
                    console.log(error);
                }
    ...
    provideLinter() {
        ...
        return {
            ...
            lint: (editor) => {
                ...
                return helpers.tempFiles(filesToValidate, (files) => helpers.exec(command, args.concat(files), {
                    stream: 'stdout',
                    ignoreExitCode: true,
                }))
                    .then((output) => (parseGlslValidatorResponse(filesToValidate, output, helpers.generateRange(editor, 0))))
                    .catch((error) => {
                    console.error(error);
                    return null;
                });
